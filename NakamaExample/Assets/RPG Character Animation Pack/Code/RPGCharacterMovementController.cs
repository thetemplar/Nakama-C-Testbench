...
namespace RPGCharacterAnims
{
    ...
    public class RPGCharacterMovementController:SuperStateMachine
    {
        ...
        {
            return true;//superCharacterController.currentGround.IsGrounded(false, 0.01f);
        }

        public bool MaintainingGround()
        {
            return true;//superCharacterController.currentGround.IsGrounded(true, 0.5f);
        }
        ...
        private void Idle_SuperUpdate()
        {
            ...
            currentVelocity = Vector3.MoveTowards(currentVelocity, Vector3.zero, groundFriction * Time.deltaTime/*superCharacterController.deltaTime*/);
        }
        ...
        private void Move_SuperUpdate()
        {
            ...
            if(rpgCharacterInputController.HasMoveInput() && canMove)
            {
                ...
                {
                    currentVelocity = Vector3.MoveTowards(currentVelocity, LocalMovement() * 1.35f, movementAcceleration * Time.deltaTime/*superCharacterController.deltaTime*/);
                    return;
                }
                //Sprinting.
                if(isSprinting)
                {
                    currentVelocity = Vector3.MoveTowards(currentVelocity, LocalMovement() * sprintSpeed, movementAcceleration * Time.deltaTime/*superCharacterController.deltaTime*/);
                    return;
                }
                //Strafing or Walking.
                if(rpgCharacterController.isStrafing)
                {
                    currentVelocity = Vector3.MoveTowards(currentVelocity, LocalMovement() * walkSpeed, movementAcceleration * Time.deltaTime/*superCharacterController.deltaTime*/);
                ...
                //Run.
                currentVelocity = Vector3.MoveTowards(currentVelocity, LocalMovement() * runSpeed, movementAcceleration * Time.deltaTime/*superCharacterController.deltaTime*/);
            }
            else
            {
                currentState = RPGCharacterState.Idle;
        ...
        private void Jump_EnterState()
        {
            ...
            currentVelocity += transform.up/*superCharacterController.up*/ * CalculateJumpSpeed(jumpHeight, gravity);
        ...
        {
            Vector3 planarMoveDirection = Math3d.ProjectVectorOnPlane(transform.up/*superCharacterController.up*/, currentVelocity);
            Vector3 verticalMoveDirection = currentVelocity - planarMoveDirection;
            if(Vector3.Angle(verticalMoveDirection, transform.up/*superCharacterController.up*/) > 90 && AcquiringGround())
            {
            ...
            planarMoveDirection = Vector3.MoveTowards(planarMoveDirection, LocalMovement() * inAirSpeed, jumpAcceleration * Time.deltaTime/*superCharacterController.deltaTime*/);
            verticalMoveDirection -= transform.up/*superCharacterController.up*/ * gravity * Time.deltaTime/*superCharacterController.deltaTime*/;
        ...
        {
            currentVelocity += transform.up/*superCharacterController.up*/ * CalculateJumpSpeed(doubleJumpHeight, gravity);
            canDoubleJump = false;
            doublejumped = true;
        ...
        private void Fall_SuperUpdate()
        {
            if(AcquiringGround())
            {
                currentVelocity = Math3d.ProjectVectorOnPlane(transform.up/*superCharacterController.up*/, currentVelocity);
                currentState = RPGCharacterState.Idle;
                rpgCharacterState = RPGCharacterState.Idle;
                return;
            }
            DoubleJump();
            currentVelocity -= transform.up/*superCharacterController.up*/ * gravity * Time.deltaTime/*superCharacterController.deltaTime*/;
        }
        ...
        {
            //Swim movement.
            currentVelocity = Vector3.MoveTowards(currentVelocity, LocalMovement() * swimSpeed, waterFriction * Time.deltaTime/*superCharacterController.deltaTime*/);
            //Apply friction to slow character to a halt on vertical axis.
            currentVelocity = Vector3.MoveTowards(currentVelocity, new Vector3(currentVelocity.x, 0, currentVelocity.z), (waterFriction * 5) * Time.deltaTime/*superCharacterController.deltaTime*/);
        ...
        {
            currentVelocity += transform.up/*superCharacterController.up*/ * CalculateJumpSpeed(1, gravity);
            animator.SetTrigger("JumpTrigger");
        }
        ...
        private void Roll_SuperUpdate()
        {
            if(rollNumber == 1)
            {
                currentVelocity = Vector3.MoveTowards(currentVelocity, transform.forward * rollSpeed, groundFriction * Time.deltaTime/*superCharacterController.deltaTime*/);
            }
            if(rollNumber == 2)
            {
                currentVelocity = Vector3.MoveTowards(currentVelocity, transform.right * rollSpeed, groundFriction * Time.deltaTime/*superCharacterController.deltaTime*/);
            }
            if(rollNumber == 3)
            {
                currentVelocity = Vector3.MoveTowards(currentVelocity, -transform.forward * rollSpeed, groundFriction * Time.deltaTime/*superCharacterController.deltaTime*/);
