...
namespace Cinemachine
{
    ...
    public class CinemachineInputProvider : MonoBehaviour, AxisState.IInputAxisProvider
    {
        ...
        /// <summary>Bool action for Mouse Button</summary>
        [Tooltip("Bool action for Mouse Button")]
        ...
        public virtual float GetAxisValue(int axis)
        {
            ...
            if (action != null)
            {
                switch (axis)
                {
                    case 0: return Mouse.current.rightButton.isPressed ? action.ReadValue<Vector2>().x : 0;// action.ReadValue<Vector2>().x;
                    case 1: return Mouse.current.rightButton.isPressed ? action.ReadValue<Vector2>().y : 0;
